...
/*
 * EXTRACTION AUDIO + TRANSCRIPTION EN PARALLÃˆLE
 */
app.post("/api/transcribe", upload.single("video"), (req, res) => {
    if (!req.file) {
        return res.status(400).json({
            success: false,
            error: "Aucune vidÃ©o reÃ§ue."
        });
    }
    ...
    /*
     * On extrait l'audio jouable immÃ©diatement.
     */
    ffmpegExtractPlayableAudio(videoPath, audioPath, (audioError) => {
        ...
        ffmpegExtractAudio(videoPath, wavPath, (wavError) => {
            ...
            if (wavError) {
                ...
                currentJob.error = "PrÃ©paration de l'audio pour la transcription impossible : " + wavError.message;
            ...
            rl.on("line", (line) => {
                try {
                    ...
                    if (event.type === "complete") {
                        ...
                        activeJob.sourceVideoPath = videoPath;
activeJob.sourceVideoUrl = `/uploads/${path.basename(videoPath)}`;
                    }
                } catch {}
            });
            ...
            python.on("close", code => {
                ...
                if (code !== 0 && !activeJob.done) {
                    activeJob.status = "error";
                    activeJob.error = stderr || `Transcription arrÃªtÃ©e (code ${code}).`;
                    activeJob.done = true;
                }
            });
        });
    ...
    /*
     * RÃ©ponse immÃ©diate : le navigateur commence Ã  suivre le job.
     */
    res.json({
        success: true,
        jobId,
        ...
        python.on(
            ...
            code => {
                ...
                if (
                    ...
                    job.error =
                        stderr ||
                        `Traduction arrÃªtÃ©e (code ${code}).`;
...
app.post("/api/translate-audio", async (req, res) => {
    try {
        ...
        const voicePreset =
            (req.body && req.body.voicePreset) ||
            "current";

        const voiceReferencePath =
            (req.body && req.body.voiceReferencePath) ||
            "";
...
app.post("/api/apply-ewe-audio", async (req, res) => {
    try {
        const transcriptionJobId = req.body?.transcriptionJobId;
        const audioJobId = req.body?.audioJobId;

        let sourceJob = transcriptionJobId
            ? jobs.get(transcriptionJobId)
            : null;

        let audioJob = audioJobId
            ? jobs.get(audioJobId)
            : null;
        ...
        if (!sourceJob) {
            for (const job of jobs.values()) {
                if (
                    job.sourceVideoPath &&
                    fs.existsSync(job.sourceVideoPath)
                ) {
                    sourceJob = job;
                    break;
                }
        ...
        if (!sourceJob || !sourceJob.sourceVideoPath) {
            return res.status(400).json({
                success: false,
                error: "Video originale introuvable."
            });
        }
        ...
        if (
            ...
            return res.status(400).json({
                success: false,
                error: "Audio Ewe synchronise introuvable."
            });
        }

        const audioPath = path.join(
            OUTPUT_DIR,
            path.basename(audioJob.audioUrl)
        );

        if (!fs.existsSync(audioPath)) {
            return res.status(400).json({
                success: false,
                error: "Fichier audio Ewe introuvable."
            });
        }
        ...
        finalJob.stage = "applying_ewe_audio";

        execFile(
            "ffmpeg",
            [
                "-y",
                "-i", sourceJob.sourceVideoPath,
                "-i", audioPath,
            ...
            (error, stdout, stderr) => {
                ...
                if (error) {
                    ...
                    finalJob.error =
                        stderr ||
                        error.message ||
                        "Impossible de creer la video finale.";
                    finalJob.done = true;
                    return;
                }
                ...
                if (!fs.existsSync(outputVideo)) {
                    finalJob.status = "error";
                    finalJob.error =
                        "La video finale n'a pas ete creee.";
                    finalJob.done = true;
                    return;
                }
...
app.get("/api/apply-ewe-audio/:jobId", (req, res) => {
    const job = jobs.get(req.params.jobId);

    if (!job) {
        return res.status(404).json({
            success: false,
            error: "Job video introuvable."
        });
    }
...
});app.post("/api/save-voice-reference", upload.single("voice"), (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({
                success: false,
                error: "Aucun Ã©chantillon vocal reÃ§u."
            });
        }
        ...
        const referencePath = path.join(
            OUTPUT_DIR,
            "my-voice-reference" + extension
        );

        fs.copyFileSync(req.file.path, referencePath);
        ...
        res.json({
            success: true,
            message: "Ã‰chantillon vocal enregistrÃ©.",
            voiceReferenceUrl:
                "/outputs/" + path.basename(referencePath)
        });
...
app.listen(PORT, () => {
    console.log(`EweVoice lancÃ© sur le port ${PORT}`);
});
